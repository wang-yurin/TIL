...
(MVC는 왜 생겨난걸까? 코드가 많아질수록 코드를 파악하기 힘들고 기능을 수정할 때마다 코드를 갈아 엎으며 유지보수가 어려웠는데 점차 유지보수가 편해지는 코드들의 패턴과 규칙성이 생겼고 MVC는 유지보수가 편해지는 코드 구성 방식으로 생각하면 된다. 따라서 유지보수를 위해 생겨난 패턴  
